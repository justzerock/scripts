    ...
    if ($.farmInfo.farmUserPro) {
      ...
      if ($.farmInfo.treeState === 2 || $.farmInfo.treeState === 3) {
        ...
        if ($.isNode() && WP_APP_TOKEN_ONE) {
          await notify.sendNotifybyWxPucher($.name, `🍉 ${$.farmInfo.farmUserPro.name} 已可领取\n👉 京东 -> 我的 -> 东东农场兑换\n🧧 红包有效期大概7天，过期作废\n⚠️ 若提示达到上限，请0点再兑换`, `${$.UserName}`);
        }
        return
        ...
        if ($.isNode() && WP_APP_TOKEN_ONE) {
          await notify.sendNotifybyWxPucher($.name, `🤔 你忘了种新水果了\n👉 京东->我的->东东农场, 随便种一个吧\n 🧧 红包可以用于京东app的任意商品.`, `${$.UserName}`);
        }
        return
      }
      await doDailyTask();
