...
$.logic = async function () {
    ...
    //console.log(info.farmWinGoods[level][0])
    //console.log('treeState:' + info?.farmUserPro?.treeState)
    if (!info.farmUserPro && !info.farmWinGoods) {
        $.log('玩不了哦')
        return
    }
    if (info.farmUserPro.treeState > 0) {
    ...
    let element = info.farmWinGoods[0];
    await $.wait(1000, 3000)
