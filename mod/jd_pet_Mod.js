    ...
    if (initPetTownRes.code === '0' && initPetTownRes.resultCode === '0' && initPetTownRes.message === 'success') {
      ...
      if ($.petInfo.petStatus === 5) {
        ...
        if ($.isNode() && WP_APP_TOKEN_ONE) {
          await notify.sendNotifybyWxPucher($.name, `🐶 ${$.petInfo.goodsInfo.goodsName} 已可领取\n 👉 京东 -> 我的 -> 东东萌宠兑换\n 🧧 红包有效期大概是三天,过期作废`, `${$.UserName}`);
        }
        return
        ...
        if ($.isNode() && WP_APP_TOKEN_ONE) {
          await notify.sendNotifybyWxPucher($.name, `🤔 你忘记选新奖品了\n 👉 京东->我的->东东萌宠, 随便选个奖品吧\n 🧧 京东红包可用于京东app的任意商品.`, `${$.UserName}`);
        }
        return
      }
