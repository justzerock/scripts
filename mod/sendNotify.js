...
if (WP_APP_TOKEN_ONE) {
  ...
  if (NowHour < 9 || NowHour > 21) {
    WP_APP_TOKEN_ONE = "";
    console.log(`只在9点后和22点前启用一对一推送，故此次暂时取消一对一推送...`);
  }
} else
...
function getPushWeek(strRemark) {
  let pushWeek = "";
  if (strRemark) {
    var Tempindex = strRemark.indexOf("@@");
    if (Tempindex != -1) {
      ...
      for (let j = 1; j < TempRemarkList.length; j++) {
        if (TempRemarkList[j]) {
          if (TempRemarkList[j].length > 4) {
            if (TempRemarkList[j].substring(0, 4).toLowerCase() == "week") {
              pushWeek = TempRemarkList[j];
              break;
            }
          }
        }
      ...
      for (let j = 1; j < TempRemarkList.length; j++) {
        if (TempRemarkList[j]) {
          if (TempRemarkList[j].length > 3) {
            if (TempRemarkList[j].substring(0, 3).toLowerCase() == "day") {
              pushDay = TempRemarkList[j];
              break;
            }
          }
        }
        ...
        if (Uid) {
          ...
          if (text == "京东资产变动") {
            try {
              ...
              let strPet = ''
              let strFactory = ''
              if (desp.indexOf('白嫖活动') != -1) {
                strReward = desp.match(/.+(\兑换|\领取)/gm).toString() || ''
                strFruit = strReward.indexOf('东东农场') != -1 ? '🧧 「东东农场」可领取\n' : ''
                strPet = strReward.indexOf('东东萌宠') != -1 ? '🧧 「东东萌宠」可领取\n' : ''
                strFactory = strReward.indexOf('京喜工厂') != -1 ? '🎁 「京喜工厂」可领取\n' : ''
              }
            ...
            WP_UIDS_ONE = ''
            if (desp.indexOf('签到失败') != -1) {
              if (desp.indexOf('签到状态不满足条件') != -1) {
                WP_UIDS_ONE = Uid
                desp = desp.replaceAll(/签到失败:签到状态不满足条件/g, '🧧 连签完成，请前往「京东极速版」提现')
                UserRemark = '🧧 提现通知@' + UserRemark + ' 🌈'
                strsummary = '👉 请前往「京东极速版」 --> 首页(或我的) --> 「签到免单」 --> 立即提现\n⏰ 请及时提现，三天内失效（从初次提醒开始）'
              } else if (desp.indexOf('明日签到') == -1 && desp.indexOf('已经签到') == -1) {
                WP_UIDS_ONE = Uid
              }
            }
          }
          if (UserRemark) {
            if (text == "京东资产变动" || text == '京东极速版签到免单') {
              text = UserRemark
            } else {
